...
class Stab2018PadaDataProcessor(Stab2018DataProcessor):
    ...
    def _read_drfs(self, mode: str = 'train'):
        ...
        for domain in domains:
            prompts_path = Path(self.experiment_dir) / "stab2018" / self.trg_domain / "prompt_annotations" / domain
...
def test_Stab2018PadaDataProcessor():
    data_processor = Stab2018PadaDataProcessor(["cloning","deathpenalty","guncontrol","marijuanalegalization",
                                                "minimumwage","nuclearenergy"],"schooluniforms", DATA_DIR, EXP_DIR)
    print()
    ...
    percent = 99.8
    for split in Stab2018PadaDataProcessor.ALL_SPLITS:
...
class Stab2018PadaDataset(Dataset):
    ...
    def _build_training_tokenized_data(data):
        ...
        for i in range(len(data["example_id"])):
            drfs_str = f"{data['domain_label'][i]} - " + f"{Stab2018PadaDataset.DRF_DELIMITER.join(data['drfs'][i])}:"
            prompt = f"{Stab2018PadaDataset.DOMAIN_PROMPT}: "
...
def test_Stab0281PadaDataset():
    ...
    for k, v in data_processor.data[split].items():
        if type(v) is not int:
            ...
            print(k, v[0], len(v[0]))
    dataset = Stab2018PadaDataset(split, data_processor, T5TokenizerFast.from_pretrained("t5-base"), 64, 0.5, 0.5)
    print(len(dataset))
